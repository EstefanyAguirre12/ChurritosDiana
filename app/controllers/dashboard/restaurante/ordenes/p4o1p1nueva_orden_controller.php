...
try{
        ...
        if($nueva_orden->setCantida($_POST['cantidad'])){
           ...
           if($nueva_orden->setIdproducto($_GET['producto'])){
                if($nueva_orden->addDettalleRestaurante()){
                    ...
                    $cuenta = $_GET['cuenta'];
                    Page::showMessage(1, "Se ha agregado correctamente se continuara con selecion de productos" , "p4nueva_orden.php?cliente=$id_cliente&cuenta=$cuenta ");
                }else{
	...
	if($data){
		//Se muestran los 
        require_once("../../../app/views/dashboard/restaurante/ordenes/p4o1p1nueva_orden_view.php");
	}else{

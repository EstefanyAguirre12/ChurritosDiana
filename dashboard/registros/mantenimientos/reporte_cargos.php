...
{ 
    ...
    // celda(ancho en cm,alto en cm,texto para mostrar,borde,ajustar celda,alineacion de la celda,color de fondo)
    $this->Cell(0,1,utf8_decode('Empleados con cargo:'.$valores[0]),0,0,'C',false);  
    // Line break
    $this->Ln(1);
}
// Page footer
...
{ //posicion 
    ...
    // Arial
    $this->SetFont('Arial','B',14); 
    $this->Cell(0,0.9,'Cargos barolo',0,2,'C',false);
    // Page number
 ...
 function tabla($header, $data)  
{
    ...
    {       
       ...
       $this->Cell($w[4],2.5,$row[5],'LR',0,'C',$fill);
       $this->Cell($w[5],2.5, $this->Image('../../../web/img/users/'.$row[6], $this->GetX(), $this->GetY(),2.5),'LR',0,'C',false);//se cargan los datos
...
session_start();
$cargo =new Cargos(); 
$data=$cargo->GetReporteCargos($_GET['idcargo']);
$cargo1=$cargo->getCargo11($_GET['idcargo']);
$fpdf->AddPage();  
$fpdf->titulo($cargo1);  
if($data!=null){ 
$fpdf->Ln(1);
...
$fpdf->SetFont('Arial',"",18);   
    ...
    $fpdf->Cell(0,1,utf8_decode('No hay ningun usuario asociado'),0,1,'C',false);
}

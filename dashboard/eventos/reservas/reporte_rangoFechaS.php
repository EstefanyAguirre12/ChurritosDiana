...
{ 
    ...
    // celda(ancho en cm,alto en cm,texto para mostrar,borde,ajustar celda,alineacion de la celda,color de fondo)
    $this->Cell(0,1,utf8_decode('Rango de fechas:'.$valores),0,0,'C',false);  
    // Line break
    $this->Ln(1);
}
// Page footer
 ...
 function tabla($header, $data)  
{
    ...
    {       
       ...
       $this->Cell($w[3],2.5,$row[4],'LR',0,'C',$fill);
       $this->Cell($w[4],2.5,$row[3],'LR',0,'C',$fill);
       $this->Cell($w[5],2.5,$row[5],'LR',0,'C',$fill);
       $this->Cell($w[6],2.5,$row[6],'LR',0,'C',$fill);
        $this->Ln();
        $fill = !$fill;
    }
    // Línea de cierre 
...
$header = array( 'Nombre Sala','N mesas','N sillas','Hora inicio','Hora fin','Fecha','Cliente'); 

//le asignamos margenes a la pagina
$fpdf->setMargins(1.1,1.1,1.1);
$fpdf->setTitle('Reservas rango fecha ');
...
session_start();
$Detalleconferencia =new Detalleconferencia(); 
$data=$Detalleconferencia->GetReporteReservaSalones($_GET['f1'], $_GET['f2']);
$nombre= 'fecha inicio:'.$_GET['f1'].' fecha final:'.$_GET['f2'];
$fpdf->AddPage();  
$fpdf->titulo($nombre);  
if($data!=null){ 
$fpdf->Ln(1);
...
$fpdf->SetFont('Arial',"",18);   
    ...
    $fpdf->Cell(0,1,utf8_decode('No hay ningun rango de fecha disponible '),0,1,'C',false);
}
